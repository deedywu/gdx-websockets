...
public class AndroidWebSocket extends AbstractWebSocket {
    ...
    public void connect() throws WebSocketException {
        ...
        long handle = createSocket(getUrl(), getProtocolsHeaderValue(), usePerMessageDeflate, insecureTls);
        if(handle == 0) {
    ...
    protected void sendBinary(byte[] packet) throws Exception {
        throw new WebSocketException("Binary websocket frames are not implemented for TeaVM Android yet.");
    }
    ...
    private static void ensurePumpRegistered() {
        if(!pumpRegistered) {
            pumpRegistered = true;
            Gdx.app.postRunnable(AndroidWebSocket::pumpSockets);
        }
    }
    ...
    private static void registerSocket(AndroidWebSocket socket) {
        if(!SOCKETS.contains(socket, true)) {
            SOCKETS.add(socket);
        }
    }

    private static void unregisterSocket(AndroidWebSocket socket) {
    ...
    @Import(name = "gdx_teavm_ws_android_supported")
    private static native boolean isNativeSupported();

    @Import(name = "gdx_teavm_ws_android_create")
    private static native long createSocket(String url, String protocolsHeader, boolean usePerMessageDeflate, boolean insecureTls);
